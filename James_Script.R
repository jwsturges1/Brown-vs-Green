...
setwd("C:/Users/Ryan/OneDrive - Coastal Carolina University/Rezek Lab/Student Files/Grace Kahmann/Mixng model stuff")

#setwd("C:/Users/rrezek/OneDrive - Coastal Carolina University/FIU/FCELTER")
...
plot_data(filename="TS3_isospace_plot", plot_save_pdf=FALSE, plot_save_png=F, mix,source,discr)
...
#               2.5%   25%   50%   75% 97.5%
#p.green.dry19 0.423 0.602 0.749 0.887 0.988
#p.brown.dry19 0.012 0.113 0.251 0.398 0.577
#p.green.wet19 0.484 0.681 0.837 0.941 0.995
#p.brown.wet19 0.005 0.059 0.163 0.319 0.516
...
plot_data(filename="SRS6_isospace_plot", plot_save_pdf=FALSE, plot_save_png=F, mix,source,discr)
...
source <- load_source_data(filename="sourcesRB10_1.csv",
                           source_factors=NULL,
                           conc_dep=FALSE,
                           data_type="means",
                           mix)
...
jags.SRS3 <- run_model(run="extreme", mix, source, discr, model_filename,
                       alpha.prior = 1, resid_err=F, process_err=T)
...
mixoutput<-ggplot(FCERB10_sumstats,aes(x=source, fill=source, width=0.8))+
  geom_boxplot(aes(lower = X25., upper = X75., middle = X50., ymin = X2.50., ymax = X97.50.), stat="identity")+
  theme_bw()+scale_fill_viridis_d(begin = 0.25)+ coord_cartesian(ylim = c( 0,1))+facet_wrap(~season)+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
...
mixoutput_bxplt_gb_TS<-ggplot(TS_sumstats_gb,aes(x=season, fill=source, width=0.8))+
  geom_boxplot(aes(lower = X25., upper = X75., middle = X50., ymin = X2.50., ymax = X97.50.), stat="identity")+
  theme_bw()+scale_fill_manual(values=c("saddlebrown",'limegreen'))+ coord_cartesian(ylim = c( 0,1))+facet_grid(site~.)+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
  scale_y_continuous(expand = c(0.01, 0))+labs(y="Dietary contribution")+coord_flip()
