...
TS3_combined_sumstats <- output_JAGS(jags.TS3 , mix, source, output_jags.TS3)
...
##combine posterior ground into brown/green (OR NAH)

TSFB_combined_sumstats <- combine_sources(jags.TSFB, mix, source, alpha.prior=1)
...
mixoutput_bxplt_gb_SRS<-ggplot(SRS_sumstats_gb,aes(x=source, fill=source, width=0.8))+
  geom_boxplot(aes(lower = X25., upper = X75., middle = X50., ymin = X2.50., ymax = X97.50.), stat="identity")+
  theme_bw()+scale_fill_manual(values=c("saddlebrown",'limegreen'))+ coord_cartesian(ylim = c( 0,1))+facet_grid(season~site)+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
  scale_y_continuous(expand = c(0.01, 0))+labs(y="Dietary contribution")
